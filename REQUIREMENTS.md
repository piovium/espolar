...
3. Generation of **Volar.js Mappings** (**not** Mozilla's SourceMap). Volar.js maps source to generated code **range to range**: we should take each AST Node that has `loc`/`range`property into a mappings to the printed code.
  - The generation of mapping should be top-down greedy, that is, if every child of a node remains *untouched*, then the parent node itself's mapping is enough; otherwise generate mappings of each child node.
  - And also, the mapping of sibling (both source and generated) *untouched* nodes should be combined into a single range
  - Import type `Mapping` from `@volar/source-map`, no need for Volar.js' `SourceMap` class 
4. The printer should be easily customizable (user can override print behavior including how mappings are generated)
